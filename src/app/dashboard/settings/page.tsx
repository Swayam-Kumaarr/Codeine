...
import { User, Code2, Clock, Trash2, RefreshCw, Check, AlertTriangle } from 'lucide-react'
...
export default function SettingsPage() {
  ...
  async function deleteAccount() {
    ...
    const { data: { user } } = await supabase.auth.getUser()
    if (!user) return
    // Delete all user data (RLS cascade expected)
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '12px', background: 'white', borderRadius: 'var(--r)', border: '1px solid #fed7d7' }}>
            ...
            {dangerConfirm === 'delete_account' ? (
              <div style={{ display: 'flex', gap: '8px' }}>
                <button onClick={deleteAccount} style={{ padding: '6px 12px', borderRadius: 'var(--r)', border: 'none', background: '#c53030', color: 'white', fontSize: '12px', cursor: 'pointer', fontFamily: 'var(--font-body)' }}>Delete everything</button>
                <button onClick={() => setDangerConfirm(null)} style={{ padding: '6px 12px', borderRadius: 'var(--r)', border: '1px solid var(--line-strong)', background: 'white', fontSize: '12px', cursor: 'pointer', fontFamily: 'var(--font-body)' }}>Cancel</button>
              </div>
            ) : (
              <button onClick={() => setDangerConfirm('delete_account')} style={{ padding: '6px 12px', borderRadius: 'var(--r)', border: '1px solid #fed7d7', background: 'transparent', color: '#c53030', fontSize: '12px', cursor: 'pointer', fontFamily: 'var(--font-body)', display: 'flex', alignItems: 'center', gap: '4px' }}>
